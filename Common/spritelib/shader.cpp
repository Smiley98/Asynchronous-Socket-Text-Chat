...
namespace spritelib
{
	...
	Shader & Shader::load_shader(Type a_shaderType, std::string a_shaderLoc)
	{
		...
		glShaderSource(m_shader, 1, bufferPtr/*&reinterpret_cast<GLchar*>(buffer)*/, GL_NONE);
	...
	void ShaderProgram::PassThrough::init_shader()
	{
		...
		s_vert->load_shader(Shader::VERTEX, "../Common/assets/PassthroughTextureVert.glsl")
			.compile_shader();

		s_frag->load_shader(Shader::FRAGMENT, "../Common/assets/PassthroughTextureFrag.glsl")
			.compile_shader();
